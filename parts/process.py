...
class Process():
    ...
    def run(self, lanes, track):
        ...
        #close = cv2.morphologyEx(dilate, cv2.MORPH_OPEN, kernel5, iterations = 5)
    ...
    def lanes(self, img):
        ...
        specialKernel = np.array([[0, 1, 1, 1, 0], [0, 1, 1, 1, 0], [0, 1, 1, 1, 0]], np.uint8)
        close = cv2.morphologyEx(img, cv2.MORPH_CLOSE, kernel5, iterations = 5)
        dilate = cv2.dilate(close, specialKernel, iterations = 5)
        lanes = cv2.morphologyEx(dilate, cv2.MORPH_ERODE, kernel5, iterations = 1)
        return lanes
    ...
    def track(self, img):
        ...
        cv2.polylines(curveImage, [np.int32(mid)], isClosed=False, color=(255), thickness=2)

        
        return cv2.bitwise_or(sobelLeft, sobelRight)
